...
void drawCar(int r, int c) {
  drawImageDMA(r, c, RACECAR_WIDTH, RACECAR_HEIGHT, racecar);
}

void drawTire(int r, int c) {
  drawImageDMA(r, c, TIRE_WIDTH, TIRE_HEIGHT, tire);
}

int collision(int boxCol, int boxRow, int carCol, int carRow) {
  if (carRow < (boxRow + 5) && (carRow + 22) > boxRow) {
    if (carCol < (boxCol + 5) && (carCol + 5) > boxCol) {
      return 1;
    }
  }
  return 0;
